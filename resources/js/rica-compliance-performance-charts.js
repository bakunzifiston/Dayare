...
function patchDonutCenterLabels() {
    const specs = window.ricaComplianceChartSpecs || [];
    const centerValue = window.ricaComplianceDonutCenter;
    ...
    specs
        ...
        .forEach((spec) => {
            ...
            chart.options.plugins.procDashCenterText = {
                ...
                subLabel: spec.centerLabel ?? 'Total reports',
                color: '#111827',
                subColor: '#6b7280',
            };
            chart.update();
...
function initRicaComplianceCharts() {
    const specs = window.ricaComplianceChartSpecs;
    ...
    if (standardSpecs.length > 0) {
        drawProcessorCharts('rica-compliance-performance', standardSpecs);
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initRicaComplianceCharts);
} else {
    initRicaComplianceCharts();
}
