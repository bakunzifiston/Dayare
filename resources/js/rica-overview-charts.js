...
function patchDonutCenterLabels() {
    const specs = window.ricaOverviewChartSpecs || [];
    const centerValue = window.ricaOverviewDonutCenter;
...
function initRicaOverviewCharts() {
    const specs = window.ricaOverviewChartSpecs;
    ...
    if (standardSpecs.length > 0) {
        drawProcessorCharts('rica-overview', standardSpecs);
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initRicaOverviewCharts);
} else {
    initRicaOverviewCharts();
}
