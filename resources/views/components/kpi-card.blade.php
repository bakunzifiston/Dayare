@props([
    ...
    'stat' => false,
    /** Tighter card for list-page summaries */
    'compact' => false,
])
...
@php
    ...
    $baseClass = $stat
        ? ($compact
            ? 'rounded-bucha border border-slate-200 bg-white px-3.5 py-3'
    ...
    $statInteractive = $stat && $href
        ? ($compact
            ? 'transition hover:border-bucha-primary/30 hover:bg-slate-50/70 focus:outline-none focus-visible:ring-2 focus-visible:ring-bucha-primary/40'
