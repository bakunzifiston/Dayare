    ...
    <div class="space-y-4">
        <section id="company-form" class="rounded-lg border border-slate-200 bg-white p-4">
            ...
            <form method="POST" action="{{ route('logistics.company.store') }}" class="grid gap-2 md:grid-cols-3">
                ...
                <input name="name" class="rounded-md border-slate-300 text-sm" placeholder="{{ __('Company name') }}" required>
                <input name="registration_number" class="rounded-md border-slate-300 text-sm" placeholder="{{ __('Registration number') }}" required>
                <input name="tax_id" class="rounded-md border-slate-300 text-sm" placeholder="{{ __('Tax ID') }}">
                <input name="license_type" class="rounded-md border-slate-300 text-sm" placeholder="{{ __('License type') }}" required>
                <input type="date" name="license_expiry_date" class="rounded-md border-slate-300 text-sm" required>
                <input name="contact_person" class="rounded-md border-slate-300 text-sm" placeholder="{{ __('Contact person') }}">
