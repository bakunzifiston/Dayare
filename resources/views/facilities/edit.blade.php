<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Edit Facility') }} — {{ $facility->facility_name }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form method="post" action="{{ route('businesses.facilities.update', [$business, $facility]) }}" class="space-y-6" id="facility-edit-form" onsubmit="var f=document.getElementById('facility-edit-form');if(f){['country_id','province_id','district_id','sector_id','cell_id','village_id'].forEach(function(id){var s=document.getElementById(id),h=f.querySelector('input[name='+id+']');if(s&&h)h.value=s?s.value:'';});}return true;">
                    @csrf
                    @method('put')

                    <div>
                        <x-input-label for="facility_name" :value="__('Facility Name')" />
                        <x-text-input id="facility_name" name="facility_name" type="text" class="mt-1 block w-full" :value="old('facility_name', $facility->facility_name)" required />
                    ...
                    <div>
                        ...
                        <select id="facility_type" name="facility_type" class="mt-1 block w-full border-gray-300 focus:border-bucha-primary focus:ring-bucha-primary rounded-md shadow-sm" required>
                            @foreach (\App\Models\Facility::selectableTypes($facility) as $t)
                    ...
                    <div>
                        ...
                        <x-text-input id="gps" name="gps" type="text" class="mt-1 block w-full" :value="old('gps', $facility->gps)" />
                        <x-input-error class="mt-2" :messages="$errors->get('gps')" />
                    </div>

                    <div>
                        <x-input-label for="license_number" :value="__('License Number')" />
                        <x-text-input id="license_number" name="license_number" type="text" class="mt-1 block w-full" :value="old('license_number', $facility->license_number)" />
                    ...
                    <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        <div>
                            ...
                            <x-text-input id="license_issue_date" name="license_issue_date" type="date" class="mt-1 block w-full" :value="old('license_issue_date', $facility->license_issue_date?->format('Y-m-d'))" />
                        ...
                        <div>
                            ...
                            <x-text-input id="license_expiry_date" name="license_expiry_date" type="date" class="mt-1 block w-full" :value="old('license_expiry_date', $facility->license_expiry_date?->format('Y-m-d'))" />
                    ...
                    <div>
                        ...
                        <x-text-input id="daily_capacity" name="daily_capacity" type="number" min="0" class="mt-1 block w-full" :value="old('daily_capacity', $facility->daily_capacity)" />
                    ...
                    <div class="flex gap-4">
                        <x-primary-button>{{ __('Update Facility') }}</x-primary-button>
    ...
    <script>
        function locationDropdownsEdit() {
            ...
            return {
                ...
                countryId: '{{ old("country_id", $facility->country_id) }}' || '',
                provinceId: '{{ old("province_id", $facility->province_id) }}' || '',
                districtId: '{{ old("district_id", $facility->district_id) }}' || '',
                sectorId: '{{ old("sector_id", $facility->sector_id) }}' || '',
                cellId: '{{ old("cell_id", $facility->cell_id) }}' || '',
                villageId: '{{ old("village_id", $facility->village_id) }}' || '',
