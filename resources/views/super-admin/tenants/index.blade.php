    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto space-y-6">
            <div class="flex flex-wrap items-center justify-between gap-3">
                ...
                @include('super-admin.tenants.partials.users-workspace-filter', [
                                ...
                                <tbody class="divide-y divide-slate-100">
                                    @foreach ($tenantRows as $tenant)
                                        <tr class="hover:bg-slate-50/70 transition-colors align-top">
                                            ...
                                            <td class="px-4 py-3.5">
                                                <a href="{{ route('super-admin.tenants.show', ['tenant' => $tenant['id']]) }}" class="font-medium text-bucha-primary hover:text-bucha-burgundy">
                                                    {{ $tenant['tenant_name'] }}
                                                </a>
                                            </td>
                                            ...
                                            <td class="px-4 py-3.5">
                                                <a href="{{ route('super-admin.tenants.show', ['tenant' => $tenant['id']]) }}#users"
                                                   class="inline-flex min-w-8 justify-center rounded-md bg-slate-100 px-2 py-1 text-xs font-semibold tabular-nums text-bucha-primary hover:bg-bucha-primary/10 hover:text-bucha-burgundy"
                                                   aria-label="{{ __('View users for :name', ['name' => $tenant['tenant_name']]) }}">
                                                    {{ $tenant['users_count'] }}
                                                </a>
                                            </td>
                                            <td class="px-4 py-3.5">
                                            ...
                                            <td class="px-4 py-3.5 text-right whitespace-nowrap">
                                                <a href="{{ route('super-admin.tenants.show', ['tenant' => $tenant['id']]) }}" class="text-bucha-primary hover:text-bucha-burgundy text-xs font-semibold">{{ __('Profile') }}</a>
                                                @if ((int) ($tenant['id'] ?? 0) !== (int) Auth::id() && auth()->user()?->hasSuperAdminModuleAccess(\App\Models\User::SUPER_ADMIN_MODULE_USERS))
                                        ...
                                        <tr class="hover:bg-slate-50/70 transition-colors">
                                            ...
                                            <td class="px-4 py-3.5 text-right">
                                                @if (! empty($row['tenant_id']))
                                                    <a href="{{ route('super-admin.tenants.show', ['tenant' => $row['tenant_id']]) }}" class="text-bucha-primary hover:text-bucha-burgundy text-xs font-semibold">{{ __('Profile') }}</a>
                                                @else
