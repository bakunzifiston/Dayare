<x-app-layout>
    <x-slot name="header">
        ...
        <h2 class="mt-1 font-semibold text-xl text-slate-800">{{ __('Edit livestock') }}</h2>
    </x-slot>
    ...
    <div class="max-w-lg space-y-6">
        ...
        <form method="post" action="{{ route('farmer.farms.livestock.update', [$farm, $livestock]) }}" class="bg-white rounded-bucha border border-slate-200/60 p-6 space-y-4">
            @csrf
            @method('put')
            <div>
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <x-text-input id="total_quantity" name="total_quantity" type="number" min="0" class="mt-1 block w-full" :value="old('total_quantity', $livestock->total_quantity)" required />
                ...
                <div>
                    ...
                    <x-text-input id="available_quantity" name="available_quantity" type="number" min="0" class="mt-1 block w-full" :value="old('available_quantity', $livestock->available_quantity)" required />
            ...
            <div class="rounded-lg border border-dashed border-slate-200 bg-slate-50/80 p-4 space-y-3">
                ...
                <div>
                    ...
                    <select name="feeding_type" id="feeding_type" class="mt-1 block w-full rounded-lg border-gray-300">
                        ...
                        @foreach (\App\Models\Livestock::FEEDING_TYPES as $f)
                            <option value="{{ $f }}" @selected(old('feeding_type', $livestock->feeding_type) === $f)>{{ ucfirst(str_replace('_', ' ', $f)) }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="health_status" id="health_status" class="mt-1 block w-full rounded-lg border-gray-300">
                        ...
                        @foreach (\App\Models\Livestock::HEALTH_STATUSES as $h)
                            <option value="{{ $h }}" @selected(old('health_status', $livestock->health_status) === $h)>{{ ucfirst($h) }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <x-text-input id="base_price" name="base_price" type="number" step="0.01" min="0" class="mt-1 block w-full" :value="old('base_price', $livestock->base_price)" />
        ...
        <form method="post" action="{{ route('farmer.farms.livestock.details.update', [$farm, $livestock]) }}" class="bg-white rounded-bucha border border-slate-200/60 p-6 space-y-4">
            @csrf
            @method('patch')
            ...
            <div>
                ...
                <x-text-input id="age_range" name="age_range" type="text" class="mt-1 block w-full" :value="old('age_range', $livestock->detail?->age_range)" placeholder="{{ __('e.g. 12–18 mo') }}" />
                <x-input-error :messages="$errors->get('age_range')" class="mt-2" />
            </div>
            <div>
                <x-input-label for="weight_range" :value="__('Weight range')" />
                <x-text-input id="weight_range" name="weight_range" type="text" class="mt-1 block w-full" :value="old('weight_range', $livestock->detail?->weight_range)" placeholder="{{ __('e.g. 400–500 kg') }}" />
                <x-input-error :messages="$errors->get('weight_range')" class="mt-2" />
            </div>
            <div>
                <x-input-label for="notes" :value="__('Notes')" />
                <textarea name="notes" id="notes" rows="3" class="mt-1 block w-full rounded-lg border-gray-300 text-sm" placeholder="{{ __('Optional notes') }}">{{ old('notes', $livestock->detail?->notes) }}</textarea>
                <x-input-error :messages="$errors->get('notes')" class="mt-2" />
            </div>
            <button type="submit" class="inline-flex items-center px-4 py-2 border border-slate-300 text-slate-800 text-sm font-semibold rounded-bucha hover:bg-slate-50">{{ __('Save details') }}</button>
        </form>
    </div>
</x-app-layout>
