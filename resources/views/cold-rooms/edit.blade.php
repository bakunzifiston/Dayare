    ...
    <div class="py-8">
        <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm sm:rounded-lg border border-slate-200/60 p-6">
                <form method="post" action="{{ route('cold-rooms.manage.update', $room) }}" class="space-y-5">
                    @csrf
                    @method('PUT')
                    <div>
                    ...
                    <div>
                        ...
                        <x-text-input id="capacity" name="capacity" type="number" step="0.01" min="0" class="mt-1 block w-full" :value="old('capacity', $room->capacity)" />
                    ...
                    <div class="flex gap-3">
                        ...
                        <a href="{{ route('cold-rooms.hub') }}" class="inline-flex items-center px-4 py-2 bg-white border border-slate-300 rounded-md font-semibold text-xs text-slate-700 uppercase tracking-widest shadow-sm hover:bg-slate-50">{{ __('Cancel') }}</a>
                    </div>
                </form>
            </div>
        </div>
