<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            ...
            <a href="{{ route('inspectors.show', $inspector) }}" class="inline-flex items-center px-4 py-2 bg-bucha-primary border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-bucha-burgundy shrink-0">{{ __('Back to inspector') }}</a>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form method="post" action="{{ route('inspectors.update', $inspector) }}" class="space-y-8" id="inspector-edit-form" onsubmit="var f=document.getElementById('inspector-edit-form');if(f){['country_id','province_id','district_id','sector_id','cell_id','village_id'].forEach(function(id){var s=document.getElementById(id),h=f.querySelector('input[name='+id+']');if(s&&h)h.value=s?s.value:'';});}return true;">
                    @csrf
                    @method('put')
                    @php
                        $cid = old('country_id', $inspector->country_id);
                        $pid = old('province_id', $inspector->province_id);
                        $did = old('district_id', $inspector->district_id);
                        $sid = old('sector_id', $inspector->sector_id);
                        $cellId = old('cell_id', $inspector->cell_id);
                        $vid = old('village_id', $inspector->village_id);
                    @endphp
                    ...
                    <div class="flex gap-4">
                        ...
                        <a href="{{ route('inspectors.hub') }}" class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:bg-gray-50">
                            {{ __('Cancel') }}
                        </a>
                    </div>
                </form>
