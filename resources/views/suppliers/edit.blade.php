<x-app-layout>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 space-y-4">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select id="business_id" name="business_id" class="mt-1 block w-full rounded-lg border-gray-300 focus:border-bucha-primary focus:ring-bucha-primary">
                            @foreach ($businesses as $business)
                                <option value="{{ $business->id }}" @selected(old('business_id', $supplier->business_id) == $business->id)>{{ $business->business_name }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select id="type" name="type" class="mt-1 block w-full rounded-lg border-gray-300 focus:border-bucha-primary focus:ring-bucha-primary">
                            @foreach (['farm', 'company', 'cooperative', 'individual'] as $type)
                                <option value="{{ $type }}" @selected(old('type', $supplier->type) === $type)>{{ ucfirst($type) }}</option>
                            @endforeach
                        </select>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <x-text-input id="first_name" name="first_name" type="text" class="mt-1 block w-full" :value="old('first_name', $supplier->first_name)" required />
                        <x-input-error class="mt-2" :messages="$errors->get('first_name')" />
                    </div>
                    <div>
                        <x-input-label for="last_name" :value="__('Last name')" />
                        <x-text-input id="last_name" name="last_name" type="text" class="mt-1 block w-full" :value="old('last_name', $supplier->last_name)" required />
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <x-text-input id="date_of_birth" name="date_of_birth" type="date" class="mt-1 block w-full" :value="old('date_of_birth', optional($supplier->date_of_birth)->format('Y-m-d'))" max="{{ date('Y-m-d') }}" />
                        <x-input-error class="mt-2" :messages="$errors->get('date_of_birth')" />
                    </div>
                    <div>
                        <x-input-label for="nationality" :value="__('Nationality')" />
                        <x-text-input id="nationality" name="nationality" type="text" class="mt-1 block w-full" :value="old('nationality', $supplier->nationality)" />
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <x-text-input id="registration_number" name="registration_number" type="text" class="mt-1 block w-full" :value="old('registration_number', $supplier->registration_number)" />
                ...
                <div class="flex items-center gap-2">
                    <input id="is_active" name="is_active" type="checkbox" value="1" class="rounded border-gray-300 text-bucha-primary focus:ring-bucha-primary" @checked(old('is_active', $supplier->is_active)) />
                ...
                <div>
                    ...
                    <select id="supplier_status" name="supplier_status" class="mt-1 block w-full rounded-lg border-gray-300 focus:border-bucha-primary focus:ring-bucha-primary">
                ...
                @php
                    ...
                    $pid = old('province_id', $supplier->province_id);
                    $did = old('district_id', $supplier->district_id);
                    $sid = old('sector_id', $supplier->sector_id);
                    $cellId = old('cell_id', $supplier->cell_id);
                    $vid = old('village_id', $supplier->village_id);
                @endphp
                <div class="mt-6 border-t border-slate-200 pt-4" x-data="supplierLocationDropdownsEdit()" x-init="loadCountries()">
                ...
                <div class="mt-4">
                    ...
                    <textarea id="notes" name="notes" rows="2" class="mt-1 block w-full rounded-lg border-gray-300 focus:border-bucha-primary focus:ring-bucha-primary">{{ old('notes', $supplier->notes) }}</textarea>
                    <x-input-error class="mt-2" :messages="$errors->get('notes')" />
                </div>
            </div>

            <div class="flex justify-between items-center">
                <form method="POST" action="{{ route('suppliers.destroy', $supplier) }}" onsubmit="return confirm('{{ __('Are you sure you want to delete this supplier?') }}');">
                    @csrf
                    @method('DELETE')
    ...
    <script>
        function supplierLocationDropdownsEdit() {
