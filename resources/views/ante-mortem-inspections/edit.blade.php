<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Edit ante-mortem inspection') }} — {{ $inspection->inspection_date->format('d M Y') }}
        </h2>
    </x-slot>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form method="post" action="{{ route('ante-mortem-inspections.update', $inspection) }}" class="space-y-6" id="ante-mortem-edit-form">
                    ...
                    <div>
                        ...
                        <textarea id="notes" name="notes" rows="3" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('notes', $inspection->notes) }}</textarea>
