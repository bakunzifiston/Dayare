...
<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div class="min-w-0 flex flex-wrap items-center gap-3">
                <div class="inline-flex items-center rounded-lg bg-gradient-to-r from-bucha-burgundy to-bucha-primary px-3 py-1.5">
                    <span class="text-xs font-bold uppercase tracking-wide text-white/90">{{ __('FPU/FRM/018') }}</span>
                </div>
                <h2 class="font-semibold text-xl text-slate-800 leading-tight">{{ __('Monthly inspection reports') }}</h2>
            </div>
        </div>
    </x-slot>
    ...
    <div class="max-w-7xl mx-auto space-y-4">
        <nav class="rica-index-tabs" aria-label="{{ __('Report views') }}">
            <a href="{{ route('monthly-inspection-reports.index', array_merge($filterParams, ['view' => 'submitted'])) }}"
               class="rica-index-tabs__link {{ $view === 'submitted' ? 'rica-index-tabs__link--active' : '' }}">
                {{ __('Submitted to RICA') }}
            </a>
            <a href="{{ route('monthly-inspection-reports.index', array_merge($filterParams, ['view' => 'facilities'])) }}"
        ...
        <section class="rica-index-filters" x-data="{ allPeriods: @js($allPeriods) }">
            <form method="GET" action="{{ route('monthly-inspection-reports.index') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 items-end">
                ...
                @endif
                <div>
                    <label class="text-xs font-medium text-slate-500" for="business_id">{{ __('Business') }}</label>
                    ...
                    <table class="rica-index-table">
                        ...
                        <tbody>
                            @forelse ($facilities as $facility)
                                ...
                                <tr>
                                    <td class="font-medium text-slate-900">{{ $facility->facility_name }}</td>
                                    ...
                                    <td class="text-right">
                                        <a href="{{ route('monthly-inspection-reports.show', ['facility' => $facility, 'month' => $monthValue]) }}"
                            ...
                            @forelse ($submittedReports as $submission)
                                ...
                                <tr>
                                    ...
                                    <td class="text-right whitespace-nowrap">
                                        @if ($facility)
                                            <a href="{{ route('monthly-inspection-reports.show', ['facility' => $facility, 'month' => $periodMonth]) }}"
                                               class="text-sm font-medium text-bucha-primary hover:text-bucha-burgundy">
                                                {{ __('View') }}
                                            </a>
                                            <span class="text-slate-300 mx-1" aria-hidden="true">·</span>
                                            <a href="{{ route('monthly-inspection-reports.pdf', ['facility' => $facility, 'month' => $periodMonth]) }}"
                                               class="text-sm font-medium text-bucha-primary hover:text-bucha-burgundy">
                                                {{ __('PDF') }}
                                            </a>
                                        @else
