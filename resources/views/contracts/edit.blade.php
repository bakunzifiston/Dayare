<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <a href="{{ route('contracts.show', $contract) }}" class="inline-flex items-center px-4 py-2 bg-bucha-primary border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-bucha-burgundy">{{ __('Back to contract') }}</a>
        </div>
    </x-slot>
        ...
        <form method="POST" action="{{ route('contracts.update', $contract) }}" class="space-y-6" enctype="multipart/form-data">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 space-y-4">
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="2" class="mt-1 block w-full rounded-lg border-gray-300 focus:border-bucha-primary focus:ring-bucha-primary">{{ old('description', $contract->description) }}</textarea>
                </div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 space-y-4">
                ...
                <div><x-input-label for="salary_payment_terms" :value="__('Salary / payment terms')" /><textarea id="salary_payment_terms" name="salary_payment_terms" rows="2" class="mt-1 block w-full rounded-lg border-gray-300 focus:border-bucha-primary focus:ring-bucha-primary">{{ old('salary_payment_terms', $contract->salary_payment_terms) }}</textarea></div>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div><x-input-label for="certification_requirements" :value="__('Certification requirements')" /><textarea id="certification_requirements" name="certification_requirements" rows="2" class="mt-1 block w-full rounded-lg border-gray-300 focus:border-bucha-primary focus:ring-bucha-primary">{{ old('certification_requirements', $contract->certification_requirements) }}</textarea></div>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <x-input-label for="signed_contract_file" :value="__('Signed contract file')" />
                        @if ($contract->signed_contract_file)
                            <p class="mt-1 text-sm text-slate-600">{{ __('Current:') }} <a href="{{ route('contracts.file.download', [$contract, 'signed', basename($contract->signed_contract_file)]) }}" class="text-bucha-primary hover:underline">{{ basename($contract->signed_contract_file) }}</a></p>
                        @endif
                    ...
                    <div>
                        ...
                        @if ($contract->supporting_documents && count($contract->supporting_documents) > 0)
                            <p class="mt-1 text-sm text-slate-600">{{ __('Current:') }} @foreach ($contract->supporting_documents as $path) <a href="{{ route('contracts.file.download', [$contract, 'supporting', basename($path)]) }}" class="text-bucha-primary hover:underline mr-2">{{ basename($path) }}</a> @endforeach</p>
                        @endif
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 space-y-4">
                ...
                <div><x-input-label for="animal_health_cert_requirement" :value="__('Animal health cert. requirement')" /><textarea id="animal_health_cert_requirement" name="animal_health_cert_requirement" rows="2" class="mt-1 block w-full rounded-lg border-gray-300 focus:border-bucha-primary focus:ring-bucha-primary">{{ old('animal_health_cert_requirement', $contract->animal_health_cert_requirement) }}</textarea></div>
                <div><x-input-label for="veterinary_inspection_requirement" :value="__('Veterinary inspection requirement')" /><textarea id="veterinary_inspection_requirement" name="veterinary_inspection_requirement" rows="2" class="mt-1 block w-full rounded-lg border-gray-300 focus:border-bucha-primary focus:ring-bucha-primary">{{ old('veterinary_inspection_requirement', $contract->veterinary_inspection_requirement) }}</textarea></div>
                <div><x-input-label for="animal_welfare_compliance" :value="__('Animal welfare compliance')" /><textarea id="animal_welfare_compliance" name="animal_welfare_compliance" rows="2" class="mt-1 block w-full rounded-lg border-gray-300 focus:border-bucha-primary focus:ring-bucha-primary">{{ old('animal_welfare_compliance', $contract->animal_welfare_compliance) }}</textarea></div>
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 space-y-4">
                <div>
                    ...
                    <textarea id="notes" name="notes" rows="3" class="mt-1 block w-full rounded-lg border-gray-300 focus:border-bucha-primary focus:ring-bucha-primary">{{ old('notes', $contract->notes) }}</textarea>
