<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <section class="grid grid-cols-2 gap-3 lg:grid-cols-4 mb-6" aria-label="{{ __('Businesses summary') }}">
                <x-kpi-card stat compact color="slate" :title="__('Total businesses')" :value="$kpis['total']" glyph="building" />
                <x-kpi-card stat compact color="bucha" :title="__('Total facilities')" :value="$kpis['facilities']" glyph="building" />
            </section>
            @if (session('status'))
                        ...
                        @foreach ($businesses as $business)
                            <li class="p-4 flex justify-between items-center hover:bg-slate-50/80 transition-colors">
                                ...
                                <div class="flex shrink-0 items-center gap-1.5">
                                    <a href="{{ route('businesses.show', $business) }}" class="inline-flex h-8 items-center rounded-lg border border-slate-200 bg-white px-3 text-xs font-medium text-slate-700 hover:bg-slate-50">{{ __('View') }}</a>
                                    <a href="{{ route('businesses.facilities.index', $business) }}" class="inline-flex h-8 items-center rounded-lg border border-slate-200 bg-white px-3 text-xs font-medium text-slate-700 hover:bg-slate-50">{{ __('Facilities') }}</a>
