...
<x-app-layout>
    <x-slot name="header">
        <div>
            ...
            <p class="mt-1 text-sm text-gray-500">{{ __('Manage abattoirs, farms, and other meat sources for your shop.') }}</p>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-6">
                    ...
                    @forelse ($suppliers as $supplier)
                        <div class="rounded-lg border border-slate-200 px-4 py-3 text-sm">
                            ...
                            @if ($supplier->district)
                                <p class="mt-1 text-slate-500">{{ $supplier->district }}@if ($supplier->sector), {{ $supplier->sector }}@endif</p>
                            @endif
                            <div class="mt-2 flex gap-3 text-xs font-semibold">
                                <a href="{{ route('butcher.suppliers.index', ['edit' => $supplier->id]) }}" class="text-bucha-primary hover:underline">{{ __('Edit') }}</a>
            ...
            <form
                method="post"
                action="{{ $editingSupplier ? route('butcher.suppliers.update', $editingSupplier) : route('butcher.suppliers.store') }}"
            ...
            >
                ...
                <div class="flex flex-wrap justify-between gap-3 pt-2">
                    @if ($editingSupplier)
                        <a href="{{ route('butcher.suppliers.index') }}" class="inline-flex items-center rounded-bucha border border-slate-200 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">
                            {{ __('Cancel edit') }}
                        </a>
                    @else
                        <span></span>
