...
<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div>
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Processing sessions') }}</h2>
                <p class="mt-1 text-sm text-gray-500">{{ __('All open and closed processing sessions.') }}</p>
            </div>
                    ...
                    <tbody>
                        @forelse ($sessions as $session)
                            <tr class="border-b border-slate-100 hover:bg-slate-50">
                                <td class="py-3 pr-4">
                                    <a href="{{ route('butcher.processing.sessions.show', $session) }}" class="font-semibold text-bucha-primary hover:underline">{{ $session->session_number }}</a>
                                </td>
                        ...
                        @empty
                            <tr><td colspan="7" class="py-6 text-center text-slate-500">{{ __('No processing sessions yet.') }}</td></tr>
                        @endforelse
                    </tbody>
                </table>
