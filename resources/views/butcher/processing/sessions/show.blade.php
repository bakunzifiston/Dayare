                ...
                <section class="rounded-bucha border border-slate-200/80 bg-white p-5 shadow-bucha">
                    ...
                    @if ($cutTypes->isEmpty())
                        <p class="mt-3 text-sm text-slate-500">
                            ...
                            <a href="{{ route('butcher.processing.types.index') }}" class="font-semibold text-bucha-primary hover:underline">{{ __('Cut type catalog') }}</a>
                        </p>
                    @else
                        <form method="post" action="{{ route('butcher.processing.sessions.outputs.store', $session) }}" class="mt-4 grid grid-cols-1 gap-4 md:grid-cols-3">
                            @csrf
                            <div>
                    ...
                    <tbody>
                        @forelse ($session->cutOutputs as $output)
                            <tr class="border-b border-slate-100">
                                ...
                                <td class="py-3">
                                    <form method="post" action="{{ route('butcher.processing.sessions.label', [$session, $output]) }}" class="inline">
                                        @csrf
