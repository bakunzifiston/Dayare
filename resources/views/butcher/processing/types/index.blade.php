<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-wrap items-center justify-between gap-3">
            ...
            <a href="{{ route('butcher.processing.index') }}" class="text-sm font-semibold text-bucha-primary hover:underline">{{ __('Back to processing') }}</a>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <section class="rounded-bucha border border-slate-200/80 bg-white p-5 shadow-bucha">
                ...
                <form method="post" action="{{ route('butcher.processing.types.store') }}" class="mt-4 grid grid-cols-1 gap-4 md:grid-cols-4">
                    @csrf
                    <div>
