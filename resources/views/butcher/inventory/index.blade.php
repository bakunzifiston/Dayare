    ...
    <x-slot name="header">
        <div class="flex flex-wrap items-center justify-between gap-3">
            ...
            <div class="flex flex-wrap gap-2">
                ...
                <a href="{{ route('butcher.inventory.disposals.index') }}" class="inline-flex items-center rounded-bucha bg-bucha-primary px-4 py-2 text-sm font-semibold text-white hover:bg-bucha-burgundy">{{ __('Log disposal') }}</a>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <section class="rounded-bucha border border-slate-200/80 bg-white p-5 shadow-bucha">
                <div class="flex items-center justify-between">
                    ...
                    <a href="{{ route('butcher.inventory.batches.index') }}" class="text-xs font-semibold text-bucha-primary hover:underline">{{ __('All batches') }}</a>
                </div>
                <div class="mt-4 space-y-3">
                    @forelse ($summary['fifo_batches'] as $batch)
                        <a href="{{ route('butcher.inventory.batches.show', $batch) }}" class="block rounded-lg border border-slate-200 px-4 py-3 hover:bg-slate-50">
