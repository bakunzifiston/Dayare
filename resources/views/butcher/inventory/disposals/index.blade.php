<x-app-layout>
    <x-slot name="header">
        <div>
            <a href="{{ route('butcher.inventory.index') }}" class="text-sm font-medium text-bucha-primary hover:text-bucha-burgundy">{{ __('← Inventory') }}</a>
    ...
    <div class="py-8">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <form method="post" action="{{ route('butcher.inventory.disposals.store') }}" class="rounded-bucha border border-slate-200/80 bg-white p-4 sm:p-6 shadow-bucha space-y-4" onsubmit="return confirm(@js(__('Record this disposal? Stock will be permanently reduced.')))">
                @csrf
