    ...
    <div class="py-6 lg:py-8">
        <div class="max-w-[1100px] mx-auto">
            <section class="rounded-bucha border border-slate-200 bg-white px-5 py-5">
                <form method="POST" action="{{ route('finance.ebm.update', $record) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mt-6 flex items-center gap-2">
                        ...
                        <a href="{{ route('finance.ebm.index') }}" class="rounded-lg border border-slate-200 px-4 py-2 text-sm">{{ __('Back') }}</a>
                    </div>
                </form>
            </section>
        </div>
