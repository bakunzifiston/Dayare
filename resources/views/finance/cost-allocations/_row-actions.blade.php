...
@if ($allocation)
    <div class="inline-flex divide-x divide-slate-200 overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
        @if ($showView)
            <a href="{{ route('finance.cost-allocations.show', $allocation) }}" class="{{ $actionClass }} text-slate-700 hover:bg-slate-50">
        ...
        @endif
        <a href="{{ route('finance.cost-allocations.edit', $allocation) }}" class="{{ $actionClass }} text-slate-700 hover:bg-slate-50">
        ...
        <form method="POST" action="{{ route('finance.cost-allocations.destroy', $allocation) }}" class="flex" onsubmit="return confirm(@js(__('Delete this cost allocation?')))">
            @csrf
            @method('DELETE')
