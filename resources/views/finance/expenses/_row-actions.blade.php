...
@if ($expense)
    <div class="inline-flex divide-x divide-slate-200 overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
        @if ($showView)
            <a href="{{ route('finance.expenses.show', $expense) }}" class="{{ $actionClass }} text-slate-700 hover:bg-slate-50">
        ...
        @endif
        <a href="{{ route('finance.expenses.edit', $expense) }}" class="{{ $actionClass }} text-slate-700 hover:bg-slate-50">
        ...
        <form method="POST" action="{{ route('finance.expenses.destroy', $expense) }}" class="flex" onsubmit="return confirm(@js(__('Delete this expense? Linked payments will also be removed.')))">
            @csrf
            @method('DELETE')
