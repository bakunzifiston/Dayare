...
@if ($payable)
    <div class="inline-flex divide-x divide-slate-200 overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
        @if ($showView)
            <a href="{{ route('finance.payables.show', $payable) }}" class="{{ $actionClass }} text-slate-700 hover:bg-slate-50">
        ...
        @endif
        <a href="{{ route('finance.payables.edit', $payable).'?tab='.$payable->payablesTabKey() }}" class="{{ $actionClass }} text-slate-700 hover:bg-slate-50">
        ...
        <form method="POST" action="{{ route('finance.payables.destroy', $payable) }}" class="flex" onsubmit="return confirm(@js(__('Delete this payable? Linked payments will also be removed.')))">
            @csrf
            @method('DELETE')
