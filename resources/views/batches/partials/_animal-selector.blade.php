<div class="border border-gray-200 rounded-md">
    ...
    <div class="p-3">
        ...
        <table class="w-full text-sm">
            ...
            <tbody>
                @foreach ($executionItems as $ei)
                    @php
                        $intake = $ei->intakeItem;
                        $alreadyBatched = in_array($ei->animal_intake_item_id, $alreadyBatchedIds ?? [], true);
                        $isChecked = is_array(old('selected_animal_ids'))
                            ? in_array($ei->animal_intake_item_id, old('selected_animal_ids'), true)
                            : ! $alreadyBatched;
                    @endphp
                    <tr class="border-t border-gray-100 {{ $alreadyBatched ? 'opacity-50' : '' }}">
                        <td class="py-1 px-2">
                            <input type="checkbox"
                                   ...
                                   value="{{ $ei->animal_intake_item_id }}"
                                   class="animal-checkbox"
                                   data-meat-kg="{{ $ei->meat_quantity_kg }}"
                        ...
                        <td class="py-1 px-2">
                            {{ number_format($ei->meat_quantity_kg, 2) }} kg
                            @if ($alreadyBatched)
