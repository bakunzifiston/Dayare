    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-slate-800 leading-tight">
                {{ __('Animal origin (intake)') }}
            </h2>
            <a href="{{ route('animal-intakes.create') }}" class="inline-flex items-center px-4 py-2 bg-[#3B82F6] border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-[#2563eb]">
                {{ __('Record intake') }}
            </a>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            @if ($intakes->isEmpty())
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-slate-200/60 p-8 text-center text-slate-600">
                    ...
                    <a href="{{ route('animal-intakes.create') }}" class="inline-flex items-center px-4 py-2 bg-[#3B82F6] border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-[#2563eb]">{{ __('Record first intake') }}</a>
                </div>
            @else
                            ...
                            <li class="p-4 flex justify-between items-center hover:bg-slate-50/80 transition-colors">
                                <div>
                                    ...
                                    <p class="text-sm text-slate-500">
                                        {{ $i->supplier_firstname }} {{ $i->supplier_lastname }} · {{ $i->farm_name ?? '—' }} · {{ $i->species }} · {{ $i->number_of_animals }} {{ __('animals') }}
                                    </p>
                                ...
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('animal-intakes.show', $i) }}" class="text-sm text-indigo-600 hover:text-indigo-900">{{ __('View') }}</a>
