<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <form method="post" action="{{ route('animal-intakes.update', $intake) }}" enctype="multipart/form-data" class="space-y-6" id="animal-intake-edit-form" onsubmit="var f=document.getElementById('animal-intake-edit-form');if(f){['country_id','province_id','district_id','sector_id','cell_id','village_id'].forEach(function(id){var s=document.getElementById(id),h=f.querySelector('input[name='+id+']');if(s&&h)h.value=s?s.value:'';});}return true;">
                @csrf
                @method('patch')
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-slate-200/60 p-6 space-y-4">
                    ...
                    <div>
                        ...
                        <select id="source_type" name="source_type" class="mt-1 block w-full border-slate-300 focus:border-bucha-primary focus:ring-bucha-primary rounded-md shadow-sm" required>
                            <option value="{{ \App\Models\AnimalIntake::SOURCE_TYPE_SUPPLIER }}" @selected(old('source_type', $intake->source_type ?? ($intake->client_id ? \App\Models\AnimalIntake::SOURCE_TYPE_CLIENT : \App\Models\AnimalIntake::SOURCE_TYPE_SUPPLIER)) === \App\Models\AnimalIntake::SOURCE_TYPE_SUPPLIER)>{{ __('Supplier') }}</option>
                            <option value="{{ \App\Models\AnimalIntake::SOURCE_TYPE_CLIENT }}" @selected(old('source_type', $intake->source_type ?? ($intake->client_id ? \App\Models\AnimalIntake::SOURCE_TYPE_CLIENT : \App\Models\AnimalIntake::SOURCE_TYPE_SUPPLIER)) === \App\Models\AnimalIntake::SOURCE_TYPE_CLIENT)>{{ __('Client') }}</option>
                        </select>
                    ...
                    <div>
                        ...
                        <select id="supplier_id" name="supplier_id" class="mt-1 block w-full border-slate-300 focus:border-bucha-primary focus:ring-bucha-primary rounded-md shadow-sm">
                            ...
                            @foreach ($suppliers as $s)
                                <option value="{{ $s->id }}" @selected(old('supplier_id', $intake->supplier_id) == $s->id)>{{ trim(($s->first_name ?? '') . ' ' . ($s->last_name ?? '')) ?: ($s->name ?? '') }}{!! $s->phone ? ' · ' . e($s->phone) : '' !!}</option>
                            @endforeach
                        </select>
                    ...
                    <div id="client-source-fields" class="space-y-4 hidden">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                ...
                                <x-text-input id="manual_client_firstname" name="manual_client_firstname" type="text" class="mt-1 block w-full" :value="old('manual_client_firstname', $intake->source_type === \App\Models\AnimalIntake::SOURCE_TYPE_CLIENT ? $intake->supplier_firstname : '')" />
                            ...
                            <div>
                                ...
                                <x-text-input id="manual_client_lastname" name="manual_client_lastname" type="text" class="mt-1 block w-full" :value="old('manual_client_lastname', $intake->source_type === \App\Models\AnimalIntake::SOURCE_TYPE_CLIENT ? $intake->supplier_lastname : '')" />
                        ...
                        <div>
                            ...
                            <x-text-input id="manual_client_contact" name="manual_client_contact" type="text" class="mt-1 block w-full" :value="old('manual_client_contact', $intake->source_type === \App\Models\AnimalIntake::SOURCE_TYPE_CLIENT ? $intake->supplier_contact : '')" />
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-slate-200/60 p-6 space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <x-text-input id="species_ear_tag" name="species_ear_tag" type="text" class="mt-1 block w-full" :value="old('species_ear_tag', $intake->species_ear_tag)" />
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                        <div>
                            ...
                            <select id="sex" name="sex" class="mt-1 block w-full border-slate-300 focus:border-bucha-primary rounded-md shadow-sm">
                                <option value="">{{ __('Select') }}</option>
                                @foreach (\App\Models\AnimalIntake::SEX_OPTIONS as $sex)
                                    <option value="{{ $sex }}" @selected(old('sex', $intake->sex) === $sex)>{{ ucfirst($sex) }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <x-text-input id="age" name="age" type="number" min="0" max="99" class="mt-1 block w-full" :value="old('age', $intake->age)" />
                    ...
                    <div>
                        ...
                        <textarea id="animal_identification_numbers" name="animal_identification_numbers" rows="2" class="mt-1 block w-full border-slate-300 focus:border-bucha-primary rounded-md shadow-sm">{{ old('animal_identification_numbers', $intake->animal_identification_numbers) }}</textarea>
                    </div>
                    @php
                        $editSourceType = old('source_type', $intake->source_type ?? ($intake->client_id ? \App\Models\AnimalIntake::SOURCE_TYPE_CLIENT : \App\Models\AnimalIntake::SOURCE_TYPE_SUPPLIER));
                    @endphp
                    <div id="movement-permit-supplier-block" class="@if($editSourceType === \App\Models\AnimalIntake::SOURCE_TYPE_SUPPLIER)@else hidden @endif">
                        <x-input-label for="movement_permit_no" :value="__('Movement permit No')" />
                        <x-text-input id="movement_permit_no" name="movement_permit_no" type="text" class="mt-1 block w-full" :value="old('movement_permit_no', $intake->movement_permit_no)" />
                        <x-input-error class="mt-2" :messages="$errors->get('movement_permit_no')" />
                    </div>
                    <div id="movement-permit-client-block" class="@if($editSourceType === \App\Models\AnimalIntake::SOURCE_TYPE_SUPPLIER) hidden @endif">
                        <x-input-label for="movement_permit_document" :value="__('Movement permit (upload)')" />
                        @if ($intake->movementPermitDocumentUrl())
                            <p class="mt-1 text-sm text-slate-600">
                                {{ __('Current file:') }}
                                <a href="{{ $intake->movementPermitDocumentUrl() }}" target="_blank" rel="noopener noreferrer" class="text-bucha-primary hover:underline">{{ __('View uploaded permit') }}</a>
                            </p>
                        @endif
                        ...
                        <p class="mt-1 text-xs text-slate-500">{{ __('Upload a new file to replace the current permit. PDF or image, max 10 MB.') }}</p>
                        <x-input-error class="mt-2" :messages="$errors->get('movement_permit_document')" />
                    </div>
                    <div>
                        <x-input-label for="observation" :value="__('Observation')" />
                        <textarea id="observation" name="observation" rows="2" class="mt-1 block w-full border-slate-300 focus:border-bucha-primary rounded-md shadow-sm">{{ old('observation', $intake->observation) }}</textarea>
                    </div>
                    <div>
                        <x-input-label for="meat_inspector_name" :value="__('Meat inspector name')" />
                        <x-text-input id="meat_inspector_name" name="meat_inspector_name" type="text" class="mt-1 block w-full" :value="old('meat_inspector_name', $intake->meat_inspector_name)" />
                    </div>
                </div>

                <div id="intake-transport-block" class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-slate-200/60 p-6 space-y-4 @if($editSourceType === \App\Models\AnimalIntake::SOURCE_TYPE_SUPPLIER)@else hidden @endif">
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-slate-200/60 p-6 space-y-4">
                    <div>
                        ...
                        <select id="status" name="status" class="mt-1 block w-full border-slate-300 focus:border-bucha-primary rounded-md shadow-sm" required>
    ...
    <script>
        ...
        window.suppliersForIntakeEdit = @json($suppliersForIntake);
        window.clientsForIntakeEdit = @json($clientsForIntake);
        document.getElementById('supplier_id')?.addEventListener('change', function () {
            var id = this.value, data = window.suppliersForIntakeEdit && window.suppliersForIntakeEdit[id];
            if (data) {
        ...
        document.getElementById('client_id')?.addEventListener('change', function () {
            var id = this.value, data = window.clientsForIntakeEdit && window.clientsForIntakeEdit[id];
            if (data) {
