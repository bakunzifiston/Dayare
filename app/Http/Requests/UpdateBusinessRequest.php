...
class UpdateBusinessRequest extends FormRequest
{
    ...
    protected function prepareForValidation(): void
    {
        ...
        if (blank($this->input('business_name'))) {
            $this->merge(['business_name' => $business->business_name ?: __('Draft business').' '.strtoupper(Str::ulid())]);
        }

        if (blank($this->input('registration_number'))) {
            $this->merge(['registration_number' => $business->registration_number ?: 'PENDING-'.strtoupper(Str::ulid())]);
        }

        if (blank($this->input('contact_phone'))) {
            $this->merge(['contact_phone' => $business->contact_phone ?: '—']);
        }

        if (blank($this->input('email'))) {
            $this->merge(['email' => $business->email ?: ($this->user()?->email ?? 'pending@example.invalid')]);
        }

        if (blank($this->input('status'))) {
            $this->merge(['status' => $business->status ?: Business::STATUS_ACTIVE]);
        }
    }
